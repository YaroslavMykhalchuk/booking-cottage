            ...
            <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:640px;background:#fff;border:1px solid #e8eaf2;border-radius:12px;overflow:hidden;">
                <tr>
                    <td style="padding:18px 20px;background:#111;color:#fff;">
                        <h1 style="margin:0;font-size:18px;line-height:1.4;">{{ __('mail.title') }}</h1>
                        <p style="margin:6px 0 0;font-size:12px;opacity:.85;">
                            {{ __('mail.order_line', [
                                'id' => $booking->id,
                                'datetime' => \Carbon\Carbon::parse($booking->created_at)->format('d.m.Y H:i')
                            ]) }}
                        </p>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="padding:20px;">
                        <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:separate;border-spacing:0 10px;">
                            <tr>
                                <td style="width:200px;color:#6b7280;font-size:13px;">{{ __('mail.cottage') }}</td>
                                <td style="font-size:14px;">
                                    <strong>{{ __('mail.cottage_no', ['n' => $booking->cottage ? 2 : 1]) }}</strong>
                                </td>
                            </tr>
                            <tr>
                                <td style="color:#6b7280;font-size:13px;">{{ __('mail.check_in') }}</td>
                                <td style="font-size:14px;">
                                    <strong>{{ \Carbon\Carbon::parse($booking->date_from)->format('d.m.Y') }}</strong>
                                </td>
                            </tr>
                            <tr>
                                <td style="color:#6b7280;font-size:13px;">{{ __('mail.check_out') }}</td>
                                <td style="font-size:14px;">
                                    <strong>{{ \Carbon\Carbon::parse($booking->date_to)->format('d.m.Y') }}</strong>
                                    <span style="color:#6b7280;font-size:12px;">{{ __('mail.checkout_note') }}</span>
                                </td>
                            </tr>
                        </table>
                        ...
                        <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:separate;border-spacing:0 10px;">
                            ...
                            <tr>
                                <td style="color:#6b7280;font-size:13px;">{{ __('mail.phone') }}</td>
                                <td style="font-size:14px;"><a href="tel:{{ preg_replace('/\s+/', '', $booking->phone) }}" style="color:#111;">{{ $booking->phone }}</a></td>
                            </tr>
                            @if(!empty($booking->note))
                                <tr>
                                    <td style="color:#6b7280;font-size:13px;vertical-align:top;">{{ __('mail.comment') }}</td>
